...
function calculatePerformanceScore(tasks) {
  ...
  if (tasks.length >= 4) {
    ...
    for (let i = 0; i < chunks; i++) {
      ...
      chunkRates.push(chunkTasks.filter(t => t.status === 'completed').length / chunkTasks.length);
    }
...
async function analyzeMemberPerformance(memberId, memberName, householdId) {
  ...
  const overallScore = calculatePerformanceScore(allTasks);
  ...
  return {
    ...
    top3Strengths: categoryScores.slice(0, 3),
...
async function run() {
  try {
    ...
    parentPort.postMessage({ success: true, data: { members: results } });
