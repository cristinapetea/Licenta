...
const CATEGORY_NAMES = {
  ...
  'laundry': 'Hanging laundry',
  'dishes': 'Starting dishwasher',
...
function categorizeTask(title) {
  ...
  if (lower.includes('laundry') || lower.includes('wash') || lower.includes('hang')) return 'laundry';
  if (lower.includes('dish') || lower.includes('plates') || lower.includes('dishwasher')) return 'dishes';
...
async function getMemberStats(memberId, householdId) {
  ...
  // Category experience & detailed stats
  ...
  Object.keys(CATEGORIES).forEach(cat => {
    ...
    const catOnTime = catCompleted.filter(t => t.completedOnTime === true);
    
    if (catTasks.length > 0) {
      categoryExperience[cat] = catCompleted.length / catTasks.length;
      
      // Calculate on-time rate - if no completedOnTime data, use completion rate as proxy
      ...
      categoryStats[cat] = {
        ...
        onTime: catOnTime.length,
        completionRate: Math.round((catCompleted.length / catTasks.length) * 100),
        onTimeRate: onTimeRate
      };
    } else {
      categoryExperience[cat] = 0;
      categoryStats[cat] = {
        total: 0,
        completed: 0,
        onTime: 0,
...
class TaskSuccessAI {
  ...
  async generateRanking(householdId) {
    ...
    for (const member of household.members) {
      ...
      // Calculează top 3 strengths
      const strengths = Object.keys(CATEGORIES)
        .map(cat => {
          ...
          return {
            category: cat,
            displayName: CATEGORY_NAMES[cat],
